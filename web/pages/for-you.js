...
export default function ForYouPage() {
  const { me: user, isInitialLoading: authLoading } = useAuth();
        ...
        <div className="pb-0 bg-transparent">
          <div className="grid grid-cols-1 min-[1001px]:grid-cols-[minmax(0,1fr)_var(--stream-width,380px)] gap-0 items-start min-h-[60vh] relative">
            ...
            <div className="min-w-0 pt-6 pr-0 pb-16 min-[1001px]:pr-9">
              ...
              <h2 className="text-[22px] font-extrabold tracking-tight mb-5 text-ink leading-tight">
                For you
              </h2>
              <HeroSection posts={posts.slice(0, 5)} />
            </div>
            <aside className="min-w-0 sticky top-[var(--header-height,88px)] h-[calc(100vh-var(--header-height,88px))] max-h-[calc(100vh-var(--header-height,88px))] overflow-y-auto flex flex-col self-start bg-bg max-[1000px]:static max-[1000px]:h-auto max-[1000px]:max-h-none max-[1000px]:overflow-visible pt-[18px] px-[22px] max-md:px-0">
              <div className="flex gap-0 mx-auto mb-4 bg-bg-secondary rounded-pill p-[3px] w-fit shrink-0 border border-line">
              ...
              {posts.slice(5).map((post) => (
                ...
                >
                  ...
                  <Link
                    href={postUrl(post)}
                    className="block text-base font-extrabold leading-snug tracking-tight text-ink mb-2 transition-colors hover:text-mint"
                  >
                    {post.title}
                  </Link>
                  {excerpt(post) ? (
                    <p className="text-sm leading-relaxed text-ink-secondary tracking-tight">
                      {excerpt(post)}
                    </p>
                  ) : null}
                </article>
